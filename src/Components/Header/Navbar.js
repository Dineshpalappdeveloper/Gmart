...
const Navbar = () => {
  ...
  useEffect(() => {
    if (token) {
      ...
      axios
        .post("https://ecommerceserver-tn9j.onrender.com/api/displayCart", {
          userId: userId,
        })
    ...
    <Nav>
      <div className={togle ? "navbar active " : "navbar"}>
        <ul className="navbar_list  ">
          ...
          <li
            onMouseEnter={() => setTogle2(true)}
            onMouseLeave={() => setTogle2(false)}
            className="navbar-link"
          >
            <NavLink className="navbar-link" to="/men">
              MEN
            </NavLink>{" "}
            <DropDown
              onMouseOver={() => setTogle2(true)}
              item={["shirt", "jeans"]}
              togle2={togle2}
              togle={setTogle}
            />
          ...
          <li
            onMouseEnter={() => setTogle3(true)}
            onMouseLeave={() => setTogle3(false)}
            className="navbar-link"
          >
            <NavLink className="navbar-link" to="/women">
              WOMEN
            </NavLink>{" "}
            <DropDown
              onMouseOver={() => setTogle3(true)}
              item={["top", "jeans"]}
              togle={setTogle}
              togle2={togle3}
            />
          ...
          <li
            onMouseEnter={() => setTogle4(true)}
            onMouseLeave={() => setTogle4(false)}
            className="navbar-link"
          >
            <NavLink className="navbar-link" to="/accessories">
              ACCESSORIES
            </NavLink>{" "}
            <DropDown
              onMouseOver={() => setTogle4(true)}
              item={["pants", "shoes"]}
              togle={setTogle}
              togle2={togle4}
            />
              ...
              <div>
                ...
                {togle ? (
                  <div className="dropDown">
                    <ul>
                      <li className="head">Welcome to Gmart Shop </li>
